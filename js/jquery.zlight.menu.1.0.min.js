;(function($,window,document,undefined){
    var pluginName='zlightMenu',
        ...
        height:'50px',
            ...
            lablColor:'#ffffff',
            lablFontSize:'13px',
            iconColor:'#ffffff',iconSize:'20px',
            mobileMainColor:'#373d41',
            mainColor:'#323231',
            secondColor:'#4f5356',
            //orange
            fontColor:'#ffffff',
    ...
    Zmenu.prototype.setStyles=function(ele){
        ...
        if(this.active){
            this.active.css(
                'background-color',
                this.op.secondColor).children('a').css('color',this.op.activeFontColor)
        };
        this.subMenu.css({
            'background-color':this.op.mainColor,
    ...
    Zmenu.prototype.createMobile=function(ele){
        ...
        this.allLinks.each(function(indx,element){
            var hrf=$(element).attr('href'),
                txt=$(element).text(),
                hclass=$(element).parents('.zlight-submenu').length,tr='';
            if(hclass>0){
                for(i=0;i<hclass;i++){tr+='-'}
            };
    ...
    Zmenu.prototype.change=function(ele){
        ...
        this.li.add(this.subli).on(
            'mouseenter mouseleave.zmenuEnter',
            function(event){
                if(event.type==='mouseenter'){
                    $(this).css(
                        'background-color',
                        ele.op.secondColor).children('a').css('color',ele.op.activeFontColor)
